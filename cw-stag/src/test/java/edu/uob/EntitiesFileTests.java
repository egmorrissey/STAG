...
final class EntitiesFileTests {
  ...
  void testBasicEntitiesFileIsReadable() {
      try {
          ...
          assertEquals("A log cabin in the woods", locationDescription);

          ArrayList<Graph> entities = firstLocation.getSubgraphs();
          Graph entity = entities.get(0);
          String entityType = entity.getId().getId();
          assertEquals("artefacts", entityType);
          ...
          assertEquals("potion", entityName);
          assertEquals("Magic potion", entityDescription);
